...
type Mode = 'analyst' | 'management' | 'about'
...
interface SkuSeries {
  ...
  window30: WindowUsage
  window60: WindowUsage
  window90: WindowUsage
}

interface ValidationSummary {
  missingStockCells: number
  nonChronological: boolean
  invalidStockCells: number
  replenishmentEvents: number
...
const state = {
  series: [] as SkuSeries[],
  mode: 'analyst' as Mode,
  validation: null as ValidationSummary | null
}
...
function formatIso(y: number, m: number, day: number): string {
...
function computeWindows(history: TimePoint[]): {
  window30: WindowUsage
  window60: WindowUsage
  window90: WindowUsage
} {
  const zero: WindowUsage = { raw: 0, workingDays: 0, adjusted: 0 }
  if (history.length === 0) {
  ...
  function windowFor(days: number): WindowUsage {
    let raw = 0
    let wd = 0
...
function parseCsv(text: string) {
  ...
  fields.forEach((f, idx) => {
    ...
    let shortDay: number | undefined
    let shortMonth: number | undefined
  ...
  if (shortHeaders.length > 0) {
    ...
    ordered.forEach(h => {
      const month = h.shortMonth as number
      const day = h.shortDay as number
  ...
  if (dateHeadersWithIso.length === 0) {
    if (errorBox) errorBox.textContent = 'No recognizable date columns found. Use ISO dates (YYYY-MM-DD) or formats like 9-Dec.'
    state.series = []
    state.validation = null
    renderAnalystTable()
    renderManagementTable()
  ...
  const dateHeadersSorted = dateHeadersWithIso
    .slice()
    .sort((a, b) => Date.parse(a.iso as string) - Date.parse(b.iso as string))
  ...
  rows.forEach(row => {
    ...
    for (let i = 1; i < dateHeadersSorted.length; i += 1) {
      ...
      const prevIso = prevHeader.iso as string
      const currIso = currHeader.iso as string
...
function computeMA(hist: TimePoint[], horizon: number, w: number): number[] {
...
function computeMape(hist: TimePoint[]): number | null {
...
function usageLabel(cls: Classification): string {
  if (cls === 'Active') return 'Regularly Used'
  if (cls === 'Low-Movement') return 'Rarely Used'
  return 'No Usage'
}

function recommendation(s: SkuSeries): string {
